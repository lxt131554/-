...
**思路：** 补齐 Excel 台账中变更控制闸和收尾审批闸，实现完整业务闭环。

**新增表（2张）：**

- `sys_change`：id, project_id, content, confirm_time, impact, status(pending/confirmed), create_user_id, create_time
- `sys_approval`：id, project_id, review_situation, fail_reason, confirm_time, create_user_id, create_time

**页面改动：**

| 操作 | 文件 | 说明 |
|------|------|------|
| 改 | ProjectDetail.vue | 新增"变更控制"区域+变更列表+新增变更对话框 |
| 新建 | ProjectApproval.vue | 成果评审与审批表单页 |

**可汇报：** 变更控制闸和收尾审批闸正式上线，Excel台账全部业务流程已在系统中实现。

---

### V0.6 — Excel 导出与最终打磨（~1周）

**功能：** 导出项目全过程台账为 Excel（接近原始台账格式），修 bug，整体打磨。
